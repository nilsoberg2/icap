...
bool ICAP::computeNodeLosses(int nodeIdx, ICAPNode* node)
{
    ...
    for (int i = 0; i < degree; i++)
    {
        ...
        if (Link[linkIdx].node1 == nodeIdx)
            downIdx = linkIdx;
        else if ((mainIdx == INVALID_IDX && Link[linkIdx].newFlow > 0.0) ||
                 latIdx != INVALID_IDX)
            mainIdx = linkIdx;
        else
            latIdx = linkIdx;
    ...
    float yDown = (float)Link[downIdx].newDepth;
    float dDown = (float)Link[downIdx].xsect.yFull;
    ...
    float qMain = (float)Link[mainIdx].newFlow;
    float dMain;
    if (Link[mainIdx].type == CONDUIT && Link[mainIdx].xsect.type == CIRCULAR)
        dMain = (float)Link[mainIdx].xsect.yFull;
    else
        dMain = dDown;
    ...
    {
        qLat = (float)Link[latIdx].newFlow;
        if (Link[latIdx].type == CONDUIT && Link[latIdx].xsect.type == CIRCULAR)
            dLat = (float)Link[latIdx].xsect.yFull;
        else
            dLat = dDown;
    }
    ...
    for (unsigned int i = 0; i < node->linkIdx.size(); i++)
    {
        ...
        if (linkIdx == mainIdx)
            ...
            updateUpstreamDepthForNode(linkIdx, Node[nodeIdx].newDepth);
    }
