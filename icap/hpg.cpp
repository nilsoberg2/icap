...
int ICAPHPG::CanInterpolateExtended(int linkIdx, double dsDepth, double flow)
{
    ...
    double dsInvert = GLINK_DSINVERT(linkIdx);
    double dsElev = dsDepth + dsInvert;
    double diam = GLINK_MAXDEPTH(linkIdx);
...
bool ICAPHPG::GetUpstream(int linkIdx, double dsDepth, double flow, double& usDepth)
{
    ...
    double depth;
    int errCode = hpg->GetUpstream(flow, dsDepth+GLINK_DSINVERT(linkIdx), depth);
...
bool ICAPHPG::GetHf(int linkIdx, double dsDepth, double flow, double& hf)
{
    hpgns::CircularHPG* hpg = GetHPG(linkIdx);
    if (hpg == NULL)
    {
    ...
    int errCode = hpg->GetHf(flow, dsDepth+GLINK_DSINVERT(linkIdx), hf);
...
bool ICAPHPG::GetVolume(int linkIdx, double dsDepth, double flow, double& volume)
{
    ...
    int errCode = hpg->GetVolume(flow, dsDepth+GLINK_DSINVERT(linkIdx), volume);
...
bool ICAPHPG::PressurizedInterpolation(int linkIdx, double dsDepth, double flow, double& usDepth_out)
{
    ...
    double roughness = GLINK_ROUGHNESS(linkIdx);
    double length = GLINK_LENGTH(linkIdx);
    double slope = GLINK_SLOPE(linkIdx);
    double diameter = GLINK_MAXDEPTH(linkIdx);
    double dsInvert = GLINK_DSINVERT(linkIdx);
    ...
    if (isSteep)
    {
        ...
        //double yCrit = hCrit + GLINK_DSINVERT(linkIdx);
...
int ICAPHPG::LoadNextHPG(char* path)
{
    if (m_currentHPG < 0)
    {
        if (! Allocate(GLINK_COUNT))
            return 1;
    }

    m_currentHPG++;
    if (m_currentHPG >= GLINK_COUNT)
        return -1; // < 0 means done

    if (GLINK_GEOMTYPE(m_currentHPG) != XSGEOM_CIRCULAR)
        return 0;
...
bool ICAPHPG::LoadSingleHPG(char* filePath)
{	
    int errCode;
    hpgns::CircularHPG* h = new hpgns::CircularHPG((char*)filePath);
    if (h == NULL)
        return false;
    else if (errCode = h->GetError())
        return false;

    this->m_list.push_back(h);
...
{
    bool result = Allocate(GLINK_COUNT);
    if (! result)
        return false;
    ...
    for (int i = 0; i < GLINK_COUNT; i++)
    {
        ...
        if (GLINK_GEOMTYPE(i) != XSGEOM_CIRCULAR)
        {
            m_list[i] = NULL;
            continue;
        }
...
bool ICAPHPG::checkAndLoadHPG(int idx, const char* dir)
{
    ...
    fileStream << "DT" << std::string(GLINK_ID(idx)) << ".txt";
    std::string file1 = fileStream.str();

    fileStream.str("");
    fileStream << GLINK_ID(idx) << ".txt";
