...
#define IS_ZERO(val)        (fabs(##val##) < ZERO)
#define IS_NOT_ZERO(val)    (fabs(##val##) >= ZERO)
...
//
{
    ...
    {                                                                          //(5.0.012 - LR)
        ...
        {                                                                      //(5.0.012 - LR)
            ...
            PumpStats[j].offCurve = 0.0;                                       //(5.0.012 - LR)
        }                                                                      //(5.0.012 - LR) 
    }                                                                          //(5.0.012 - LR)
...
//
{
    ...
    if ( Node[j].type != OUTFALL )                                             //(5.0.012 - LR)
    {
        // Removed because ICAP changes the definition of flooding.
        //if ( newDepth >= Node[j].fullDepth + Node[j].surDepth )
        //{
        //    NodeStats[j].timeFlooded += tStep;
        //    NodeStats[j].maxPondedVol = MAX(NodeStats[j].maxPondedVol,         //(5.0.012 - LR)
        //                                (newVolume - Node[j].fullVolume));     //(5.0.010 - LR)
        //    NodeStats[j].volFlooded += Node[j].overflow * tStep;               //(5.0.012 - LR)
        //}
        if ( newDepth + Node[j].invertElev + FUDGE >= Node[j].crownElev )      //(5.0.012 - LR)
        {                                                                      //(5.0.012 - LR)
            NodeStats[j].timeSurcharged += tStep;                              //(5.0.012 - LR)
        }                                                                      //(5.0.012 - LR)
    }
...
//
{
    ...
    else if ( Link[j].oldDepth > LinkStats[j].maxDepth )
    {
        LinkStats[j].maxDepth = Link[j].oldDepth;
    }
    ...
    if ( Link[j].type == PUMP )
    {
        ...
        {                                                                      //(5.0.012 - LR)
            ...
            PumpStats[k].offCurve += Link[j].flowClass*tStep;                  //(5.0.012 - LR)
