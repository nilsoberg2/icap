...
typedef struct
{
   ...
   int           canFlood;        // can this node be flooded
   ...
   double        overflow;        // ICAP now uses this as the head above the full depth (was overflow rate (cfs))
...
typedef struct
{
    ...
    double       gateHt;          // height of gate opening
}   TGate;
...
{                                  // All routing totals are in ft3.
   ...
   double        floodingNumNodes; // number of nodes flooded
   double        outflow;          // external outflow
   double        pumpedVol;        // pumped volume
   double        initStorage;      // initial storage volume
   double        finalStorage;     // final storage volume
   double        internalOutflow;  // totalInflow - totalOutflow
}  TRoutingTotals;
