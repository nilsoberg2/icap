...
extern int FileExists(const char* strFilename); // comes from ICAP's util.cpp
...
//
{
    ...
    switch ( k )
    {
      ...
      case HPG_PATH:
		if (FileExists(s2))
	        sstrncpy(HPGPath, s2, MAXFNAME);
		else
			return error_setInpError(ERR_HPG_PATH, s2);
        break;

	  case PUMPING_RATE:
		PumpingRate = atof(s2);

	  case DAYS_BEFORE_PUMPING:
		DaysBeforePumping = atof(s2);

	  case PUMPING_THRESHOLD:
		PumpingThreshold = atof(s2);
...
//
{
   ...
   PumpingRate = 0.0; // ICAP
   DaysBeforePumping = 0.0; // ICAP
   PumpingThreshold = 0.0; // ICAP
...
//
{
    ...
    Gate   = (TGate *)   calloc(Nlinks[GATE],     sizeof(TGate));
