...
void saveStats(int* nNodeStats, int* nLinkStats)
{
    int i = 0;
    float val;
    float maxDepth = -1e20f, maxInflow = 0.0f, maxLatFlow = 0.0f, maxVol = 0.0f;
    float maxFlow = 0.0f, maxVel = -1e20f;

    for (i = 0; i < Nobjects[NODE]; i++)
    {
        val = (float)NodeStats[i].maxDepth;
        if (val > maxDepth)
            maxDepth = val;
        val = (float)NodeStats[i].maxInflow;
        if (val > maxInflow)
            maxInflow = val;
        val = (float)NodeStats[i].maxLatFlow;
        if (val > maxLatFlow)
            maxLatFlow = val;

        if (Node[i].type == STORAGE)
        {
            val = (float)(StorageStats[i].maxVol);
            if (val > maxVol)
                maxVol = val;
        }
    }
    ...
    *nNodeStats = 3;
    //fwrite(&maxDepth, sizeof(float), 1, Fout.file);
    fwrite(&maxInflow, sizeof(float), 1, Fout.file);
    fwrite(&maxLatFlow, sizeof(float), 1, Fout.file);
    fwrite(&maxVol, sizeof(float), 1, Fout.file);

    for (i = 0; i < Nobjects[LINK]; i++)
    {
        val = (float)LinkStats[i].maxFlow;
        if (val > maxFlow)
            maxFlow = val;
        val = (float)LinkStats[i].maxVeloc;
        if (val > maxVel)
            maxVel = val;
    }

    *nLinkStats = 2;
    fwrite(&maxFlow, sizeof(float), 1, Fout.file);
    fwrite(&maxVel, sizeof(float), 1, Fout.file);
}

/*
// This save stats for each link/node rather than a global value.
void saveStats(int* nNodeStats, int* nLinkStats)
{
    int i = 0;
    float maxDepth, maxInflow, maxLatFlow, maxVol;
    float maxFlow, maxVel;

    for (i = 0; i < Nobjects[NODE]; i++)
    {
        maxDepth = (float)NodeStats[i].maxDepth;
        maxInflow = (float)NodeStats[i].maxInflow;
        maxLatFlow = (float)NodeStats[i].maxLatFlow;
        maxVol = 0.0;

        if (Node[i].type == STORAGE)
            maxVol = (float)StorageStats[i].maxVol;
    ...
    for (i = 0; i < Nobjects[LINK]; i++)
    {
        maxFlow = (float)LinkStats[i].maxFlow;
        maxVel = (float)LinkStats[i].maxVeloc;
...
//
{
    int k, nNodeStats, nLinkStats;
    int statsStartPos;
    
    statsStartPos = (int)ftell(Fout.file);

    saveStats(&nNodeStats, &nLinkStats);

    k = MAGICNUMBER;
	fwrite(&k, sizeof(int), 1, Fout.file);
    fwrite(&statsStartPos, sizeof(int), 1, Fout.file);
    fwrite(&nNodeStats, sizeof(int), 1, Fout.file);
    fwrite(&nLinkStats, sizeof(int), 1, Fout.file);
...
//
{
    ...
    SysResults[SYS_FLOODING_NUM_NODES] = (float) (StepFlowTotals.floodingNumNodes);
