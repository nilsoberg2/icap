...
//
{
    ...
    fprintf(Frpt.file, "\n  CSO Volume ...............%14.3f%14.3f",
            totals->internalOutflow*ucf1, totals->internalOutflow*ucf2);

    //fprintf(Frpt.file, "\n  Number Of Overflows.......%14.3f",           //(5.0.012 - LR)
    //        totals->floodingNumNodes);

    fprintf(Frpt.file, "\n  Pumped Volume ............%14.3f%14.3f",
            totals->pumpedVol * ucf1, totals->pumpedVol * ucf2);
...
void report_QualErrors(int p1, int p2, TRoutingTotals QualTotals[])
{
    ...
    for (p = p1; p <= p2; p++)
    {
        fprintf(Frpt.file, "%14.3f", QualTotals[p].pumpedVol);
    }
    ...
    for (p = p1; p <= p2; p++)
    {
        fprintf(Frpt.file, "%14.3f", QualTotals[p].internalOutflow);
    }
    WRITE("");
}
