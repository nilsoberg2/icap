...
//
{
    ...
    Node[j].canFlood   = TRUE; // used to determine if this node can flood
    Node[j].surDepth   = 0.0;
...
//
{
    ...
    // Sometimes, because of ICAP, the depths may be negative.  So
    // here clamp the depths to a non-negative number.
    d1 = Node[j].oldDepth;
    if (d1 < 0.0)
        d1 = 0.0;
    d2 = Node[j].newDepth;
