...
//
{
    ...
    case MOD_BASKET:
        if ( p[1] <= 0.0 || p[0] < p[1] / 2.0 ) return FALSE;
        xsect->yFull = p[0]/ucf;
        xsect->wMax  = p[1]/ucf;

        // --- depth of bottom section
        xsect->yBot  = xsect->yFull - (xsect->wMax / 2.0); 

        // --- full area = area of bottom rectangle + 1/2 area of top circle
        xsect->aFull = xsect->yBot * xsect->wMax +
                       (PI/4.) * xsect->wMax * xsect->wMax / 2.;

        // --- full wetted perim = wetted perim of bottom rectangle +
        //                         1/2 circumference of top circle
        xsect->rFull = xsect->aFull /
                       (xsect->wMax + 2.0 * xsect->yBot + PI * xsect->wMax / 2.0);
        xsect->sFull = xsect->aFull * pow(xsect->rFull, 2./3.);

        // --- area corresponding to max. section factor (96% full)
        aMax = xsect->yBot * xsect->wMax +
               (PI/4.0) * xsect->wMax * xsect->wMax * (0.96 - 0.5);
        xsect->sMax = aMax * pow(mod_basket_getRofA(xsect, aMax), 2./3.);
        break;
...
//
{
    ...
    else if ( xsect->type == IRREGULAR ) return xsect->aBot;
...
double rect_round_getYofA(TXsect* xsect, double a)
{
    ...
    if ( alpha < 0.04 ) return (2.0 * xsect->rBot) * circ_getYofA(xsect, alpha);
...
{
    double y, arg, theta1;
    double theta = 2.0 * asin(xsect->wMax / 2.0 / xsect->rBot); 
    double p;
    ...
    {
        // wetted perimeter without contribution of top surface
        p = a / (2.0 * (a - xsect->aBot) /
               xsect->wMax + xsect->rBot * theta);
    ...
    y = rect_round_getYofA(xsect, a);
    arg =  y / xsect->rBot;
    if ( arg >= 1.0 ) theta1 = PI / 2.;
    else              theta1 = asin(arg);
    return a / (theta1 * (2.0 * xsect->rBot));
}
...
double rect_round_getSofA(TXsect* xsect, double a)
{
    ...
    {
        return xsect->sBot *
            circ_getSofA(xsect, (a/(PI*(xsect->rBot)*(xsect->rBot))));
    }
}
...
double rect_round_getAofY(TXsect* xsect, double y)
{
    ...
    // --- otherwise find angle of circular invert subtended by water level
    theta1 = 2.0 * acos(1.0 - y / xsect->rBot);

    // --- then use formula for area of partially full circle
    return (theta1 - sin(theta1)) * xsect->rBot * xsect->rBot / 2.0;
}

double rect_round_getRofY(TXsect* xsect, double y)
{
    double a, arg, theta1, p;
    ...
    // --- otherwise first find area
    a = rect_round_getAofY(xsect, y);

    // --- then wetted perimeter
    arg =  y / xsect->rBot;
    if ( arg >= 1.0 ) theta1 = PI / 2.;
    else              theta1 = asin(arg);
    p = theta1 * (2.0 * xsect->rBot);
...
{
    if ( y > xsect->yBot ) return xsect->wMax;        // above bottom section
    else return 2.0 * xsect->rBot *                   // below top section
                sin( acos(1.0 - y / xsect->rBot) );
}
...
double mod_basket_getYofA(TXsect* xsect, double a)
{
    ...
    if ( a <= xsect->yBot * xsect->wMax ) return a / xsect->wMax;

    // --- find unfilled area  / area of full circular top
    //     (remember full circle has diameter = wMax)
    alpha = (xsect->aFull - a) / (PI * xsect->wMax * xsect->wMax / 4.);

    // --- for small area, use analytical circular function to find
    //     unfilled height and return difference of this from full height
    if ( alpha < 0.04 ) return xsect->yFull - xsect->wMax *
                               circ_getYofA(xsect, alpha);

    // --- otherwise lookup height of unfilled area from the circular table
    y1 = xsect->wMax * lookup(alpha, Y_Circ, N_Y_Circ);

    // --- and return difference between full height & unfilled height
...
double mod_basket_getRofA(TXsect* xsect, double a)
{
    ...
    if ( a <= xsect->yBot * xsect->wMax )
        return a / (xsect->wMax + 2. * a / xsect->wMax);

    // --- find height of water above rectangular bottom
    y1 = mod_basket_getYofA(xsect, a) - xsect->yBot;

    // --- find angle of circular arc corresponding to this height
    arg = 2.0 * y1 / xsect->wMax;
    if ( arg >= 1.0 ) theta = PI / 2.;
    else              theta = asin(arg);

    // --- return area / (wetted perim of rect + arc length)
    return a / (2. * xsect->yBot + xsect->wMax + theta * xsect->wMax);
}
...
double mod_basket_getdSdA(TXsect* xsect, double a)
{
    ...
    if ( a <= xsect->yBot * xsect->wMax && a/xsect->aFull > 1.0e-30 )
    {
...
double mod_basket_getAofY(TXsect* xsect, double y)
{
    ...
    // --- water level below top of rectangular bottom; return depth * width
    if ( y <= xsect->yBot ) return y * xsect->wMax;

    // --- find unfilled height relative to diameter of circular section
    y1 = (xsect->yFull - y) / xsect->wMax;

    // --- get area of unfilled section
    alpha = invLookup(y1, Y_Circ, N_Y_Circ);
    return xsect->aFull - alpha * (PI * xsect->wMax * xsect->wMax / 4.);
}
...
double mod_basket_getWofY(TXsect* xsect, double y)
{
    ...
    // --- find unfilled depth relative to diameter of circular section
    y1 = (xsect->yFull - y) / xsect->wMax;

    // --- find normalized width from table lookup
    return xsect->wMax * lookup(y1, W_Circ, N_W_Circ);
}
