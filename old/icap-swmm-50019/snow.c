...
void snow_validateSnowmelt(int j)
{
    ...
    // --- check that removal fractions sum to <= 1.0
...
//
{
    ...
    if ( snowpack->fArea[SNOW_PLOWABLE] > 0.0 )
    {
        ...
        exc = snowpack->wsnow[SNOW_PLOWABLE] - Snowmelt[k].weplow;
        if ( exc > 0.0 )
        {
            // --- plow out of system
            ...
            snowpack->wsnow[SNOW_PLOWABLE] = Snowmelt[k].weplow +
                                             exc * (1.0 - sfracTotal);
        }
    }
}
