...
//
{
    switch ( type )
    {
      ...
      case GATE:  return outlet_readParams(j, k, tok, ntoks);
      default: return 0;
    }
}
...
//
{
    ...
    else
    {
        ...
        {                                                                      //(5.0.010 - LR)
            ...
            Link[j].xsect.yFull = x[0] / UCF(LENGTH);                          //(5.0.010 - LR)                                //(5.0.010 - LR) 
        }                                                                      //(5.0.010 - LR)
...
//
{
    ...
    switch (type)
    {
      ...
      case GATE:
        Link[j].offset1      = 0.0;
        Link[j].offset2      = Link[j].offset1;
        Gate[k].gateHt     = x[0] / UCF(LENGTH);
        Gate[k].qCoeff     = x[1];
        Gate[k].qExpon     = x[2];
        Gate[k].qCurve     = (int)x[3];
...
//  Input:   j = link index
//  Output:  none
//  Purpose: converts offset elevation to offset height for a link.
//
{
    double elev;
    elev = Node[Link[j].node1].invertElev;
    if ( Link[j].offset1 < elev ) Link[j].offset1 = 0.0;
    else Link[j].offset1 -= elev;
    Link[j].offset1 = MAX(Link[j].offset1, 0.0);
    if ( Link[j].type == CONDUIT )
    {
        elev = Node[Link[j].node2].invertElev;
        if ( Link[j].offset2 < elev ) Link[j].offset2 = 0.0;
        else Link[j].offset2 -= elev;
        Link[j].offset2 = MAX(Link[j].offset2, 0.0);
    }
...
//
{
    ...
    switch ( Link[j].type )
    {
      ...
      case GATE:  return outlet_getInflow(j);
...
//
{
    ...
    if ( y >= yMax ) y = yMax;              // don't let y be > yFull          //(5.0.010 - LR)
    if ( xsect_isOpen(xsect->type) )        // use hyd. depth for open channel //(5.0.010 - LR)
    {
...
//
{
	...
	if ( Link[j].offset2 < 0.0 ) Link[j].offset2 = 0.0;                        //(5.0.012 - LR)
    ...
    // --- compute modified slope, roughness
    Conduit[k].slope /= lengthFactor;
    roughness = roughness / sqrt(lengthFactor);                                //(5.0.010 - LR)
...
//           k = pump index
//  Output:  none
//  Purpose: initializes pump conditions at start of a simulation              //(5.0.010 - LR) 
//
{
    int    m;                          // pump curve index
    int    n1;                         // upstream node index
    double vft3;                       // volume (ft3)
    double v;                          // volume (ft3 or m3)
    double q;                          // pump flow (not used)

    // --- assign wet well volume to inlet node of Type 1 pump                 //(5.0.010 - LR)
    ...
    // --- set initial pump setting                                            //(5.0.010 - LR)
    Link[j].setting = Pump[k].initSetting;                                     //(5.0.010 - LR)
    Link[j].targetSetting = Pump[k].initSetting;                               //(5.0.010 - LR)
}
...
{                                                                              //(5.0.010 - LR)
    ...
    // --- find effective orifice discharge ceoff.                             //(5.0.012 - LR)
...
//
{
    ...
    // --- return flow through the orifice
    return dir * orifice_getFlow(j, k, head, f, Link[j].hasFlapGate);
}
...
///////////////////////////////////////////////////////////
    ...
    Link[j].ID = id;
    link_setParams(j, GATE, n1, n2, k, x);
    return 0;
}
...
//
{
    ...
    else return Gate[k].qCoeff * pow(h, Gate[k].qExpon) / UCF(FLOW);
}
