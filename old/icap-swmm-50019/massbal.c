...
//  massbal_addNodeEvap         (called from findEvap in routing.c)
...
//
{
    ...
    StepFlowTotals.floodingNumNodes  = 0.0;
    StepFlowTotals.outflow   = 0.0;
    StepFlowTotals.pumpedVol = 0.0;
    for (j=0; j<Nobjects[POLLUT]; j++)
    {
        ...
        StepQualTotals[j].floodingNumNodes  = 0.0;
        StepQualTotals[j].outflow   = 0.0;
        StepQualTotals[j].pumpedVol = 0.0;
    }
}
...
//
{
    ...
    if ( w >= 0.0 )
    {
        if ( isFlooded ) StepQualTotals[p].floodingNumNodes += w;
...
//
//  Input:   evapLoss = evaporation loss from all nodes (ft3)
//  Output:  none
//  Purpose: adds evaporation loss over current time step to routing totals.
//
{
    //StepFlowTotals.pumpedVol += evapLoss;                                        //(5.0.012 - LR)
    //FlowTotals.pumpedVol += evapLoss;
}
...
//
{
    ...
    FlowTotals.floodingNumNodes += StepFlowTotals.floodingNumNodes;
    ...
    for (j = 0; j < Nobjects[POLLUT]; j++)
    {
        ...
        QualTotals[j].floodingNumNodes += StepQualTotals[j].floodingNumNodes * tStep;
        QualTotals[j].outflow  += StepQualTotals[j].outflow * tStep;
        QualTotals[j].pumpedVol  += StepQualTotals[j].pumpedVol * tStep;
    }
...
//
{
    ...
    sysFlows[SYS_FLOODING_NUM_NODES] = (f1 * OldStepFlowTotals.floodingNumNodes +
                             f * StepFlowTotals.floodingNumNodes) * UCF(FLOW);
...
//
{
    ...
    for (j = 0; j < Nobjects[SUBCATCH]; j++)
    {
        ...
        {
            finalStorage += Subcatch[j].subArea[i].depth *
                            Subcatch[j].subArea[i].fArea;
        }
        RunoffTotals.finalStorage += finalStorage * Subcatch[j].area;
...
//
{
    ...
    totalOutflow = StepFlowTotals.outflow +
                   StepFlowTotals.pumpedVol;
