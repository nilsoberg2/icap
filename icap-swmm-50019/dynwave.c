...
//   for conduits.
//
//   All previous change comments were removed to improve readability.
//
//-----------------------------------------------------------------------------
...
static double checkFlapGate(int j, int n1, int n2, double q);
static double checkNormalFlow(int j, double q, double y1, double y2, double a1,
              double a2, double r1);

static void   setNodeDepth(int node, double dt);
static void   setWetWellVolume(int i, int canPond, double dV, double dt);
static double getFloodedDepth(int i, int canPond, double dV, double yMax,
              double dt);
...
//
{
    ...
    if ( q > 0.0 )
    {
        ...
        else if ( y1 < Link[j].xsect.yFull )
            q = checkNormalFlow(j, q, y1, y2, a1, a2, r1);
    }
...
double checkFlapGate(int j, int n1, int n2, double q)
//
//  Input:   j = link index
...
//
{
    ...
    {
        if ( q * (double)Link[j].direction < 0.0 ) return 0.0;
    }
...
//           a2 = flow area at downstream end (ft2)
...
//
{
    ...
    if ( NormalFlowLtd == SLOPE || NormalFlowLtd == BOTH ||
         Node[n1].type == OUTFALL || Node[n2].type == OUTFALL )
    {
        if ( y1 < y2 ) check = TRUE;
    }

    // --- check if Fr >= 1.0
    if ( (NormalFlowLtd == FROUDE || NormalFlowLtd == BOTH) &&
          Node[n1].type != OUTFALL && Node[n2].type != OUTFALL )
    {
        if ( y1 > FUDGE && y2 > FUDGE )
        {
...
//
{
    ...
    {
        setWetWellVolume(i, canPond, dV, dt);
        return;
    }
...
void setWetWellVolume(int i, int canPond, double dV, double dt)
//
//  Input:   i  = node index
//           canPond = TRUE if node can pond overflows
...
//
{
    double  vNew = Node[i].oldVolume + dV;
    vNew = MAX(0.0, vNew);
    Node[i].overflow = 0.0;
    if ( vNew > Node[i].fullVolume )
    {
        Node[i].overflow = (vNew - MAX(Node[i].oldVolume,
    ...
    Xnode[i].dYdT = fabs(Node[i].newDepth - Node[i].oldDepth) / dt;
}


//=============================================================================

double getFloodedDepth(int i, int canPond, double dV, double yMax, double dt)
//
//  Input:   i  = node index
...
//
{
    double vNew = Node[i].oldVolume + dV;       // new volume of water (ft3)

    // --- determine overflow lost from system
    Node[i].overflow = (vNew - MAX(Node[i].oldVolume, Node[i].fullVolume))
                       / dt;
    if ( Node[i].overflow < FUDGE ) Node[i].overflow = 0.0;
    
    // --- determine actual new volume and depth
    if ( canPond == FALSE )
    {
        Node[i].newVolume = Node[i].fullVolume;
        return yMax;
    ...
    {
        Node[i].newVolume = vNew;
        if ( Node[i].newVolume <= Node[i].fullVolume ) return yMax;
        else return Node[i].fullDepth +
                    (Node[i].newVolume - Node[i].fullVolume) /
                     Node[i].pondedArea;
    }
}
...
//
{
    ...
    if ( Conduit[k].a1 >= Link[j].xsect.aFull )
    {
        ...
        h1 = Node[n1].newDepth + Node[n1].invertElev;
        h2 = Node[n2].newDepth + Node[n2].invertElev;
        if ( (h1 - h2)/Conduit[k].modLength > fabs(Conduit[k].slope) )
