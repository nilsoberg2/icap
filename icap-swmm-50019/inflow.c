...
//  Local functions
//-----------------------------------------------------------------------------
static double getPatternFactor(int p, int month, int day, int hour);
...
//     nodeID  FLOW      tSeriesID  (FLOW         1.0          scaleFactor  baseline)
//     nodeID  pollutID  tSeriesID  (CONCEN/MASS  unitsFactor  scaleFactor  baseline)
//
{
    int    j;                          // object index
...
//
{
    ...
    {
        p = inflow->patterns[i];
        if ( p >= 0 ) f *= getPatternFactor(p, month, day, hour);
    }
