...
//
{
    ...
    // ---  check links 
    // disabled checking because ICAP/ITM can have negative slopes, and outlets
    // can be attached to normal junctions (since we're treating them different).
    /*
    for (j=0; j<Nobjects[LINK]; j++)
    {
        ...
        switch ( Link[j].type )
        {
          ...
          case CONDUIT:
              // For ICAP/ITM purposes, the slope can be negative.
            node2 = Link[j].node2;
...
//
{
    ...
    if ( Link[j].type == CONDUIT )
    {
        ...
        Link[j].newVolume = a * Conduit[k].length * Conduit[k].barrels;        //(5.0.013 - LR)
